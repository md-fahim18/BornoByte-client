...
// Fetch approved courses from backend
useEffect(() => {
    ...
    {filteredCourses.map(course => (
      <div key={course._id} className="card bg-base-200 hover:shadow-xl shadow-md transition duration-300 rounded-xl">
        ...
        <div className="card-body">
          ...
          <p className="text-sm font-medium mt-1">Teacher: <span className="text-orange-500">{course.teacherEmail}</span></p>

          <div className="flex justify-between items-center mt-4">
            <span className="flex items-center gap-1 text-sm"><FiClock /> {course.duration}</span>
            <span className="flex items-center gap-1 text-sm text-yellow-500"><FiStar /> 4.5</span>
          </div>
